...
class SvcTask(BaseTask):
    ...
    def build_tts_model(self):
        if hparams["diff_decoder_type"] == "wavenet":
            if hparams["diff_type"] == "ddpm":
                self.model = GaussianDiffusion(
                    phone_encoder=self.phone_encoder,
                    out_dims=hparams["diff_decoder_args"]["wavenet"]["in_dims"],
        ...
        elif hparams["diff_decoder_type"] == "lynxnet":
            if hparams["diff_type"] == "ddpm":
                self.model = GaussianDiffusion(
                    phone_encoder=self.phone_encoder,
                    out_dims=hparams["diff_decoder_args"]["lynxnet"]["in_dims"],
                    denoise_fn=DIFF_DECODERS[hparams['diff_decoder_type']](hparams),
                    timesteps=hparams['K_step'],
                    K_step=hparams['K_step_infer'],
                    loss_type=hparams['diff_loss_type'],
            ...
            raise ValueError(f"'{hparams['diff_decoder_type']}' is not a valid Diffusion denoiser")
