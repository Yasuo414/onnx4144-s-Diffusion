...
def get_pitch_rmvpe(wav_data, mel, hparams):
    global rmvpe_model
    ...
    if rmvpe_model is None:
        rmvpe_model = RMVPE("checkpoints/rmvpe/model.pt", hop_length=160)
    ...
    f0 = rmvpe_model.infer_from_audio(wav16k, sr, device=device, thred=0.03, use_viterbi=hparams["use_rmvpe_viterbi"])
    uv = f0 == 0

    if len(f0[~uv]) > 0:
        f0[uv] = np.interp(np.where(uv)[0], np.where(~uv)[0], f0[~uv])
    
    original_time = 0.01 * np.arange(len(f0))
    target_time = (np.arange(num_frames) * hop_size) / hparams["audio_sample_rate"]
    f0_interp = np.interp(target_time, original_time, f0)

    uv_interp = np.interp(target_time, original_time, uv.astype(float)) > 0.5
    f0_interp[uv_interp] = 0

    pitch_coarse = f0_to_coarse(f0_interp, hparams)

    return f0_interp, pitch_coarse
...
class File2Batch:
    ...
    def temporary_dict2processed_input(item_name, temp_dict, encoder, infer=False, **kwargs):
        ...
        def get_pitch(wav, mel):
            ...
            global f0_dict
            use_crepe = kwargs.get('use_crepe', hparams.get('use_crepe', False)) if infer else hparams.get('use_crepe', False)
            use_rmvpe = kwargs.get('use_rmvpe', hparams.get('use_rmvpe', False)) if infer else hparams.get('use_rmvpe', False)
            use_parselmouth = kwargs.get("use_parselmouth", hparams.get("use_parselmouth", False)) if infer else hparams.get("use_parselmouth", False)
            ...
            if use_crepe:
                ...
                md5 = get_md5(wav)
                if infer and md5 in f0_dict.keys():
                    print("load temp rmvpe f0")
                    gt_f0 = np.array(f0_dict[md5]["f0"])
                    coarse_f0 = np.array(f0_dict[md5]["coarse"])
                else:
                    torch.cuda.is_available() and torch.cuda.empty_cache()
                    gt_f0, coarse_f0 = get_pitch_rmvpe(wav, mel, hparams)
                if infer:
                    f0_dict[md5] = {"f0": gt_f0.tolist(), "coarse": coarse_f0.tolist(), "time": int(time.time())}
                    write_temp("./infer_tools/f0_temp.json", f0_dict)
            elif use_parselmouth:
