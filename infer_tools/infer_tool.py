...
from modules.diff.reflow import RectifiedFlow
from modules.diff.lynxnet import LYNXNet
...
class Svc:
    def __init__(self, project_name, config_name, hubert_gpu, model_path, onnx=False):
        ...
        self.DIFF_DECODERS = {
            "wavenet": lambda hp: DiffNet(hp["diff_decoder_args"]["wavenet"]["in_dims"]),
            "lynxnet": lambda hp: LYNXNet(**hp["diff_decoder_args"]["lynxnet"])
        }
        ...
        if hparams["diff_type"] == "ddpm":
            self.model = GaussianDiffusion(
                ...
                out_dims=self.DIFF_DECODERS[hparams["diff_decoder_type"]](hparams)["in_dims"],
                denoise_fn=self.DIFF_DECODERS[hparams['diff_decoder_type']](hparams),
                timesteps=hparams["K_step"],
                K_step=hparams["K_step_infer"],
                loss_type=hparams['diff_loss_type'],
                spec_min=hparams['spec_min'],
                spec_max=hparams['spec_max'],
            )
        elif hparams["diff_type"] == "reflow":
            ...
            raise ValueError(f"'{hparams['diff_type']}' is not a valid Diffusion process")
        
        utils.load_ckpt(self.model, self.model_path, 'model', force=True, strict=True)
        self.model.cuda()
        self.vocoder = NsfHifiGAN()

    def infer(self, in_path, key, acc, spk_id=0, use_crepe=None, use_rmvpe=None, use_parselmouth=None, singer=False):
        if use_crepe is not None or use_rmvpe is not None or use_parselmouth is not None:
            batch = self.pre(in_path, acc, spk_id, use_crepe=use_crepe, use_rmvpe=use_rmvpe, use_parselmouth=use_parselmouth)
        else:
            batch = self.pre(in_path, acc, spk_id)
        ...
        def diff_infer():
            ...
            diff_outputs = self.model(
                hubert=batch['hubert'].cuda(),
                spk_embed_id=spk_embed.cuda(),
                mel2ph=batch['mel2ph'].cuda(),
                f0=batch['f0'].cuda(),
                energy=energy,
                ref_mels=batch["mels"].cuda(),
                infer=True
            )
    ...
    def pre(self, wav_fn, accelerate, spk_id=0, use_crepe=None, use_rmvpe=None, use_parselmouth=None):
        ...
        temp_dict = File2Batch.temporary_dict2processed_input(item_name, temp_dict, self.hubert, infer=True,
                                                              use_crepe=use_crepe, use_rmvpe=use_rmvpe, use_parselmouth=use_parselmouth)
        hparams['diff_speedup'] = accelerate
