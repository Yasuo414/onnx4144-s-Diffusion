...
def run_clip(raw_audio_path, svc_model, key, acc, use_crepe, use_rmvpe, use_parselmouth, spk_id=0, auto_key=False, out_path=None, slice_db=-40,
             **kwargs):
    ...
    for (slice_tag, data) in audio_data:
        ...
        if slice_tag:
            ...
            _f0_tst, _f0_pred, _audio = svc_model.infer(raw_path, spk_id=spk_id, key=key, acc=acc, use_crepe=use_crepe, use_rmvpe=use_rmvpe, use_parselmouth=use_parselmouth)
...
if __name__ == '__main__':
    # 工程文件夹名，训练时用的那个
    project_name = "HR"
    model_path = f"checkpoints/{project_name}/model_ckpt_steps_104000.ckpt"
    config_path = f"checkpoints/{project_name}/config.yaml"

    # 支持多个wav/ogg文件，放在raw文件夹下，带扩展名
    file_names = ["first_Unnamed_Track.wav"]
    spk_id = 0
    # 自适应变调（仅支持单人模型）
    auto_key = False
    ...
    for f_name, tran in zip(file_names, trans):
        ...
        run_clip(raw_audio_path=audio_path, svc_model=model, key=tran, acc=accelerate, use_crepe=False, use_rmvpe=True, use_parselmouth=False,
