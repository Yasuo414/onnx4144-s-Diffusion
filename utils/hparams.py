...
def set_hparams(config='', exp_name='', hparams_str='', print_hparams=True, global_hparams=True, reset=True,
    ...
    if print_hparams and global_print_hparams and global_hparams:
        print(f'{colorama.Fore.YELLOW}| Hparams chains{colorama.Style.RESET_ALL}: {colorama.Fore.CYAN}{config_chains}{colorama.Style.RESET_ALL}')
        keys = ""
        keys += "| Hparams: "
        for i, (k, v) in enumerate(sorted(hparams_.items())):
            print(f"{colorama.Fore.YELLOW}{k}{colorama.Style.RESET_ALL}: {colorama.Fore.CYAN}{v}{colorama.Style.RESET_ALL}, ", end="\n" if i % 5 == 4 else "")
        print("")
