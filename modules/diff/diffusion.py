...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample_ddim(self, x, t, interval, cond):
        noise_pred = self.denoise_fn(x, t, cond=cond)
        x_start = self.predict_start_from_noise(x, t, noise=noise_pred)

        t_prev = torch.max(t - interval, torch.zeros_like(t))

        alpha_cumprod_t_prev = extract(self.alphas_cumprod, t_prev, x.shape)

        pred_dir = (1. - alpha_cumprod_t_prev - 1e-5).sqrt() * noise_pred
        x_prev = alpha_cumprod_t_prev.sqrt() * x_start + pred_dir
        return x_prev
    ...
    def forward(self, hubert, mel2ph=None, spk_embed_id=None, ref_mels=None, f0=None, energy=None, infer=False):
        ...
        if not infer:
            ...
            if hparams.get('diff_speedup') and hparams['diff_speedup'] > 1:
                iteration_interval = hparams["diff_speedup"]
                accelerator = hparams.get("diff_accelerator", "ddim")
                if accelerator == "plms":
                    self.noise_list = deque(maxlen=4)
                    for i in tqdm(reversed(range(0, t, iteration_interval)), desc="Sample time step (PLMS)", total=t // iteration_interval):
                        x = self.p_sample_plms(x, torch.full((b,), i, device=device, dtype=torch.long), iteration_interval, cond)
                elif accelerator == "ddim":
                    for i in tqdm(reversed(range(0, t, iteration_interval)), desc="Sample time step (DDIM)", total=t // iteration_interval):
                        x = self.p_sample_ddim(x, torch.full((b,), i, device=device, dtype=torch.long), iteration_interval, cond)
                else:
                    raise ValueError(f"'{accelerator}' is not a valid Diffusion sampler")
            else:
                for i in tqdm(reversed(range(0, t)), desc='Sample time step (DDPM)', total=t):
